...
ANSWER: The Python environment has several data objects, functions, package imports and variables while the R environment only has the rock dataset imported.
...
##r chunk
py$df_housing
colnames(py$df_housing)

# Option 1
py$df_housing$HouseAge[1:10]

# Option 2
py$df_housing["HouseAge"][1:10,]

# Option 3
head(py$df_housing$HouseAge, n=10)
```
...
```{python}
##python chunk
# General access. None of the variable names carry over
r.rock.columns

# Options 1 and 2 from R can also be used in python
r.rock.area[0:10]
