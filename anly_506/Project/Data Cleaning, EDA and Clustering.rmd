...
# download.file("https://raw.githubusercontent.com/owid/covid-19-data/master/public/data/owid-covid-data.csv", destfile = "covid_data.csv")
...
covid_data_cn <- covid_data[covid_data$continent=='',] # world-level data
```
...
We can also look at the deaths for each region
```{r}
total_deaths_plot <- ggplot(regions, aes(x=date, y=total_deaths, color=location)) +
  geom_line() + 
  theme_bw() +
  labs(x = 'Date', y = 'Total Deaths', title ='Total Deaths per Continent')
total_deaths_plot
```
